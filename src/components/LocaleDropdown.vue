<template>
  <div class="relative">
    <button @click="isOpen = !isOpen" class="relative z-10 block h-8 w-8 rounded-full overflow-hidden border-2 border-gray-600 focus:outline-none focus:border-white">
      <img class="h-full w-full object-cover" :src="getFlagUrl($i18n.locale)" alt="Locale flag">
    </button>
    ...
    <div v-if="isOpen" class="absolute right-0 mt-2 py-2 w-48 bg-white rounded-lg shadow-xl">
      <button v-for="locale in locales"
        ...
        @click="(ev) => {selectLocale(locale)}"
        :disabled="$i18n.locale === locale"
      >
        <div class="h-8 w-8 rounded-full overflow-hidden mr-2">
          <img class="h-full w-full object-cover" :src="getFlagUrl(locale)" alt="Locale flag">
        </div>
        {{$t(`lang.${locale}`)}}
      </button>
    </div>
